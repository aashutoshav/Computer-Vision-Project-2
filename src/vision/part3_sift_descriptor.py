...
def get_gradient_histogram_vec_from_patch(
    ...
    for r_cell in range(0, 16, cell_width):
        for c_cell in range(0, 16, cell_width):
            cell_mags = window_magnitudes[r_cell:r_cell+cell_width, c_cell:c_cell+cell_width].flatten()
            cell_oris = window_orientations[r_cell:r_cell+cell_width, c_cell:c_cell+cell_width].flatten()
...
def get_feat_vec(
    ...
    up = feature_width // 2 - 1      
    down = feature_width // 2      
    left = feature_width // 2 - 1  
    right = feature_width // 2     
    
    r_start, r_end = r_int - up, r_int + down + 1 
    c_start, c_end = c_int - left, c_int + right + 1
    
    patch_magnitudes = magnitudes[r_start:r_end, c_start:c_end]
    patch_orientations = orientations[r_start:r_end, c_start:c_end]
    
    fv = get_gradient_histogram_vec_from_patch(patch_magnitudes, patch_orientations)
...
def get_SIFT_descriptors(
    ...
    for i in range(len(X)):
        ...
        fv = get_feat_vec(c, r, magnitudes, orientations, feature_width)
        
        feature_vectors_list.append(fv.flatten())
        
    if feature_vectors_list:
        fvs = np.array(feature_vectors_list)
    else:
        fvs = np.empty((0, 128))
