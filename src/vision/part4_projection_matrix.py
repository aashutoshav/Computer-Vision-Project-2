...
def projection(P: np.ndarray, points_3d: np.ndarray) -> np.ndarray:
    ...
    projected_points_homo = P @ points_3d_homo.T

    s = projected_points_homo[2, :]
    s[s == 0] = 1e-12
    
    px = projected_points_homo[0, :] / s
    py = projected_points_homo[1, :] / s

    projected_points_2d = np.vstack((px, py)).T
